...
`insert`、`remove`、`sort` 等方法只修改列表，不输出返回值——返回的默认值为 `None` 。[1](https://docs.python.org/zh-cn/3/tutorial/datastructures.html#id2) 这是所有 Python 可变数据结构的设计原则。
...
- [1](https://docs.python.org/zh-cn/3/tutorial/datastructures.html#id1)、别的语言可能会返回可变对象，允许方法连续执行，例如，`d->insert("a")->remove("b")->sort();`。